...
class LeapfrogSolver:
    def __init__(self, alpha_cumprods, timesteps=1000, leapfrog_steps=50):
        # Leapfrog sampling parameters
        step_ratio = timesteps // leapfrog_steps
        self.leapfrog_steps = (np.arange(1, leapfrog_steps + 1) * step_ratio).round().astype(np.int64) - 1
        self.leapfrog_alpha_cumprods = alpha_cumprods[self.leapfrog_steps]
        self.leapfrog_alpha_cumprods_prev = np.asarray(
            [alpha_cumprods[0]] + alpha_cumprods[self.leapfrog_steps[:-1]].tolist()
        )
        # Convert to torch tensor
        self.leapfrog_steps = torch.from_numpy(self.leapfrog_steps).long()
        self.leapfrog_alpha_cumprods = torch.from_numpy(self.leapfrog_alpha_cumprods)
        self.leapfrog_alpha_cumprods_prev = torch.from_numpy(self.leapfrog_alpha_cumprods_prev)

    def to(self, device):
        self.leapfrog_steps = self.leapfrog_steps.to(device)
        self.leapfrog_alpha_cumprods = self.leapfrog_alpha_cumprods.to(device)
        self.leapfrog_alpha_cumprods_prev  = self.leapfrog_alpha_cumprods_prev.to(device)
        return self

    def leapfrog_step(self, pred_x0, pred_noise, timestep_index):       
        # test24 - h=0.5
        alpha_cumprod_prev = extract_into_tensor(self.leapfrog_alpha_cumprods_prev, timestep_index, pred_x0.shape)
        x_0 = (alpha_cumprod_prev).sqrt()*pred_x0
        v_0= (1.0 - alpha_cumprod_prev).sqrt() * pred_noise
        v_half = 2*v_0
        h=0.5
        x_prev = x_0+h*v_half
        return x_prev
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="The number of timesteps to use for Leapfrog sampling.",
    )
...
def main(args):
    ...
    # Initialize the Leapfrog ODE solver for distillation.

    solver = LeapfrogSolver(
        noise_scheduler.alphas_cumprod.numpy(),
        timesteps=noise_scheduler.config.num_train_timesteps,
        leapfrog_steps=args.num_leapfrog_steps
    )
    ...
    #print("Length of train data loader: ", len(train_dataloader))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # For the Leapfrog solver, the timestep schedule is [T - 1, T - k - 1, T - 2 * k - 1, ...]
 
                topk = noise_scheduler.config.num_train_timesteps // args.num_leapfrog_steps
                index = torch.randint(0, solver.leapfrog_steps.size(0), (bsz,), device=latents.device).long()
                start_timesteps = solver.leapfrog_steps[index]
                ...
                with torch.no_grad():
                    with torch.autocast("cuda"):
                        ...
                        # Note that the Leapfrog step depends on both the predicted x_0 and source noise eps_0.
                        x_prev = solver.leapfrog_step(pred_x0, pred_noise, index)
